...
class DCRNN(nn.Module, Seq2SeqAttrs):
    ...
    def __init__(self, adj_mx, **model_kwargs):
        ...
        #NOTE STDMAE fl layers:
        self.fc_his_t = nn.Sequential(nn.Linear(96, 512), nn.ReLU(), nn.Linear(512, 358), nn.ReLU())
        self.fc_his_s = nn.Sequential(nn.Linear(96, 512), nn.ReLU(), nn.Linear(512, 358), nn.ReLU())
    ...
    def decoder(self, encoder_hidden_state, contextual_features, labels=None, batches_seen=None):
        batch_size = encoder_hidden_state.size(1)
        go_symbol = torch.zeros(
            (batch_size, self.num_nodes * self.decoder_model.output_dim)).to(encoder_hidden_state.device)
        #NOTE ----------------- QUI ----------------------
        hidden_states_t = self.fc_his_t(contextual_features[:,:,:96])        # B, N, D
        hidden_states_t = hidden_states_t.unsqueeze(1)
        
        hidden_states_s = self.fc_his_s(contextual_features[:,:,96:])        # B, N, D
        ...
        projected_features = go_symbol + hidden_states_t
        projected_features = projected_features + hidden_states_s
        # ------------------------------------------------

        decoder_hidden_state = encoder_hidden_state
        decoder_input = projected_features
    ...
    def forward(self, history_data: torch.Tensor, contextual_features:torch.Tensor, future_data: torch.Tensor = None, batch_seen: int = None, **kwargs) -> torch.Tensor:
        ...
        outputs = self.decoder(encoder_hidden_state, contextual_features, future_data,
